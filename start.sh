...
TAG=v0.21.0 if [ "$BLOCKSTACK_TAG" ]; then
   TAG="$BLOCKSTACK_TAG" fi CORETAG="$TAG-browser" coreimage=quay.io/blockstack/blockstack-core:$CORETAG 
browserimage=quay.io/blockstack/blockstack-browser:$TAG if [ "$CORE_IMAGE" ]; then
    coreimage="$CORE_IMAGE" fi if [ "$BROWSER_IMAGE" ]; then
    browserimage="$BROWSER_IMAGE" fi
# Default to setting blockstack to debug mode
if [ "$BLOCKSTACK_DEBUG" != "0" ]; then
    BLOCKSTACK_DEBUG=1 fi
...
tmpdir=/tmp/.blockstack_tmp if [ ! -e $tmpdir ]; then
   mkdir -p $tmpdir fi
# set password blank so we know when to prompt
password=0 if [ "$WIN_HYPERV" == '1' ]; then
...
Docker containers mount your /home/<user>/.blockstack directory EOF
}
prompt-new-password () {
...
containers
           ...
           Legal characters:
               letters (upper and lowercase), numbers, '_', and '-' EOF
...
http://localhost:8888/auth?authRequest=\$(echo "%u" | sed s/blockstack://)' Name=Blockstack-Browser MimeType=x-scheme-handler/blockstack; EOF
